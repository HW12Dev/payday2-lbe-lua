...
function UnitNetworkHandler:send_statistics(total_kills, total_specials_kills, total_head_shots, accuracy, downs, sender)
	if not self._verify_gamestate(self._gamestate_filter.any_end_game) and not self._verify_gamestate(self._gamestate_filter.any_ingame) then
		return
	end
...
function UnitNetworkHandler:sync_peer_score(peer_id, kills, downs, accuracy, loot, small_loot, sender)
	if not self._verify_gamestate(self._gamestate_filter.any_ingame) then
		return
	end
	...
	managers.job:set_peer_stats(peer_id, {
		kills = kills,
		downs = downs,
		accuracy = accuracy,
		loot = loot,
		small_loot = small_loot
	})
end

function UnitNetworkHandler:sync_team_stats(kill, bag, small_loot, hostages_remaining, downed, sender)
	if not self._verify_gamestate(self._gamestate_filter.any_ingame) then
		return
	end
...
function UnitNetworkHandler:sync_arcade_state(state_index, sender)
	if not self._verify_gamestate(self._gamestate_filter.any_ingame) then
		return
	end

	local state = JobManager.arcade_states[state_index]

	if not state then
		return
	end

	managers.job:set_arcade_state(state)
end

function UnitNetworkHandler:sync_team_name(index, color, sender)
	if not self._verify_gamestate(self._gamestate_filter.any_ingame) then
		return
	end

	managers.job:sync_arcade_team_name(index, color)
end
