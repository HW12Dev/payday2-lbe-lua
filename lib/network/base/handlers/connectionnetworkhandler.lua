...
function ConnectionNetworkHandler:discover_host_reply(data, sender)
	if not self._verify_in_client_session() then
		return
	end

	managers.network:on_discover_host_reply(sender, data)
end
...
function ConnectionNetworkHandler:sync_server_only(sender)
	local peer = self._verify_sender(sender)
	...
	peer:set_server_only()
end

function ConnectionNetworkHandler:sync_drop_in_point(id, peer_id, pos, rot, sender)
	local peer = self._verify_sender(sender)
	...
	managers.criminals:add_custom_drop_in_point(id, pos, rot)
	managers.criminals:custom_drop_in_point_peer_id_sync(id, peer_id)
end

function ConnectionNetworkHandler:sync_drop_in_point_used(id, peer_id, sender)
	local peer = self._verify_sender(sender)
