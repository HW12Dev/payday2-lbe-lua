...
function BaseNetworkSession:init()
	...
	self._ids_STEAM = Idstring("STEAM")
	self._ids_LBE = Idstring("LBE")
	self._peers = {}
	self._peers_all = {}
...
function BaseNetworkSession:create_local_peer(load_outfit)
	...
	if SystemInfo:distribution() == Idstring("LBE") then
		local ip = Network:hostname_ip_lookup(Network:hostname())

		if #ip > 0 then
			my_user_id = ip[1] .. ":" .. tostring(NetworkManager.DEFAULT_PORT)
		end
	end
...
function BaseNetworkSession:amount_of_players()
	local amount = table.size(self._peers_all)

	if Network:is_server() and Global.game_host or Network:is_client() and managers.network:session():server_peer():server_only() then
		amount = amount - 1
	end
...
function BaseNetworkSession:chk_send_connection_established(name, user_id, peer)
	...
	elseif SystemInfo:platform() == Idstring("XB1") then
		...
		self:remove_connection_from_trash(rpc)
		self:remove_connection_from_soft_remove_peers(rpc)
	elseif SystemInfo:distribution() == self._ids_LBE then
		peer = peer or self:peer_by_user_id(user_id)
		local rpc = Network:handshake(user_id, nil, "TCP_IP")
...
function BaseNetworkSession:send_steam_p2p_msgs(wall_t)
	...
	if SystemInfo:distribution() ~= self._ids_STEAM then
		return
	end
...
function BaseNetworkSession:resolve_new_peer_rpc(new_peer, incomming_rpc)
	if SystemInfo:platform() ~= self._ids_WIN32 or SystemInfo:distribution() == self._ids_LBE then
...
function BaseNetworkSession:on_statistics_recieved(peer_id, peer_kills, peer_specials_kills, peer_head_shots, accuracy, downs)
	...
	for id, peer in pairs(self._peers_all) do
		if peer:has_statistics() and not peer:server_only() then
			local stats = peer:statistics()
			local peer_data = {
				accuracy = stats.accuracy,
				kills = managers.statistics:session_killed_by_peer(id),
				downs = managers.statistics:session_peer_downed(id)
			}
			local loot = #managers.loot:loot_by_peer_id(id)
			local small_loot = #managers.loot:loot_by_peer_id(id, true)
			peer_data.loot = loot
			peer_data.small_loot = small_loot

			managers.job:set_peer_stats(id, peer_data)
		end
	end

	managers.job:calculate_team_score()
	managers.job:calculate_team_cash()
end
