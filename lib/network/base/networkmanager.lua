...
require("lib/network/matchmaking/NetworkAccountLBE")
...
require("lib/network/matchmaking/NetworkMatchMakingLBE")
...
elseif SystemInfo:distribution() == Idstring("LBE") then
	NetworkManager.DEFAULT_PORT = 9898
else
...
if SystemInfo:platform() == Idstring("X360") or SystemInfo:platform() == Idstring("PS3") or SystemInfo:platform() == Idstring("PS4") or SystemInfo:platform() == Idstring("XB1") or SystemInfo:distribution() == Idstring("LBE") then
...
function NetworkManager:init()
	...
	elseif self._is_win32 then
		...
		elseif SystemInfo:distribution() == Idstring("LBE") then
			self.account = NetworkAccountLBE:new()
...
function NetworkManager:_create_lobby()
	if self._is_win32 then
		...
		elseif SystemInfo:distribution() == Idstring("LBE") then
			self.matchmake = NetworkMatchMakingLBE:new()
		else
...
function NetworkManager:on_discover_host_received(sender)
	...
	managers.network.matchmake:on_discover_host_received(sender)
end

function NetworkManager:on_discover_host_reply(host, data)
	print("on_discover_host_reply", data)

	if self._discover_hosts_cb then
		local cb = self._discover_hosts_cb

		self._session:on_host_discovered(host, data)
		cb(host, data)
	end
end
...
function NetworkManager:host_game()
	...
	self._session:create_local_peer(not Global.game_host)
