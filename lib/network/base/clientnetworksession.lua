...
function ClientNetworkSession:on_join_request_reply(reply, my_peer_id, my_character, level_index, difficulty_index, state_index, server_character, user_id, mission, job_id_index, job_stage, alternative_job_stage, interupt_job_stage_level_index, xuid, auth_ticket, sender)
	...
	if SystemInfo:platform() == self._ids_WIN32 and SystemInfo:distribution() == Idstring("STEAM") then
...
function ClientNetworkSession:on_host_discovered(new_host, data)
	if self._discovered_hosts then
		...
		for i_host, host_data in ipairs(self._discovered_hosts) do
			if host_data.rpc:ip_at_index(0) == new_host:ip_at_index(0) and host_data.rpc:ip_at_index(0) == new_host:ip_at_index(0) then
