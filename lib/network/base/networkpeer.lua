...
function NetworkPeer:sync_data(peer)
	...
	if Network:is_server() and Global.game_host then
		peer:send_queued_sync("sync_server_only")
	end
...
function NetworkPeer:spawn_unit(spawn_point_id, is_drop_in, spawn_as)
	...
	if is_drop_in then
		managers.criminals:sync_drop_in_points()
		managers.job:sync_arcade_team_name_to_drop_in(self)
		managers.job:sync_score_to_drop_in(self)
		managers.job:sync_arcade_state_to_drop_in(self)
	end
...
function NetworkPeer:_update_equipped_armor()
	...
	if self._equipped_armor_id ~= new_armor_id then
		if tweak_data.vr_arcade.forced_armor then
			new_armor_id = tweak_data.vr_arcade.forced_armor
		end
...
function NetworkPeer:set_server_only()
	self._server_only = true
end

function NetworkPeer:server_only()
	if managers.network:session():local_peer() == self and Global.game_host then
		return true
	end

	return self._server_only
end
