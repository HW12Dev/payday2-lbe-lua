...
function RaycastWeaponBase:init(unit)
	...
	self._trail_effect_table = {
		effect = Idstring("effects/particles/weapons/weapon_trail_lbe"),
		position = Vector3(),
		normal = Vector3()
	}
...
function RaycastWeaponBase:use_ammo(base, ammo_usage)
	...
	if is_player and tweak_data.vr_arcade.infinite_reserve_ammo then
		ammo_usage = 0
	end
...
function RaycastWeaponBase:_fire_raycast(user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, shoot_through_data)
	...
	if alive(self._obj_fire) then
		...
		for i = 1, self:weapon_tweak_data().tracers or 1 do
			local trail = World:effect_manager():spawn(self._trail_effect_table)

			if col_ray then
				World:effect_manager():set_remaining_lifetime(trail, math.clamp(col_ray.distance / 10000, 0, col_ray.distance))
			end
		end
	end
