...
function CopActionHurt:init(action_desc, common_data)
	...
	elseif action_type == "heavy_hurt" and (self._ext_anim.run or self._ext_anim.sprint) and not common_data.is_suppressed and not crouching then
		...
		if redirect then
			if not self._unit:anim_state_machine():enabled() then
				tag_print("CopActionHurt", "Animation state machine was for some reason disabled, enabling")
				self._unit:anim_state_machine():set_enabled(true)
			end
