...
function ScopeCamera:reload_material(path, material)
	Application:data_compile({
		target_db_name = "all",
		send_idstrings = false,
		preprocessor_definitions = "preprocessor_definitions_vr_production",
		verbose = false,
		platform = string.lower(SystemInfo:platform():s()),
		source_root = Application:base_path() .. "../../assets",
	...
	DB:reload()
	Application:reload_material_config(Idstring(path .. material))
	Application:set_material_texture(self._material, self._texture_channel, self._render_target)
end
