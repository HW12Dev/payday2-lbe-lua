...
function PlayerDamage:update(unit, t, dt)
	if self._arcade_invulnerable_t and self._arcade_invulnerable_t < t then
		self:set_invulnerable(false)

		self._arcade_invulnerable_t = nil
	end

	if self._regen_t and self._regen_t < t then
		self:set_health(self:_max_health() * tweak_data.vr_arcade.regen_amount)
...
function PlayerDamage:down_time()
	return tweak_data.vr_arcade.respawn_time
end
...
function PlayerDamage:_calc_armor_damage(attack_data)
	...
	if self:health_ratio() < tweak_data.vr_arcade.regen_limit then
		self._regen_t = TimerManager:game():time() + tweak_data.vr_arcade.regen_time
	end
...
function PlayerDamage:revive(helped_self)
	...
	if not self._unit:movement():has_teleport_data("state") then
		local peer = managers.network:session() and managers.network:session():local_peer()
		...
		self._unit:movement():trigger_teleport(teleport_data)
	end
	...
	self._arcade_invulnerable_t = TimerManager:game():time() + tweak_data.vr_arcade.respawn_grace_period
