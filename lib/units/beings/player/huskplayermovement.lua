...
function HuskPlayerMovement:set_need_revive(need_revive, down_time)
	...
	if need_revive then
		...
		managers.statistics:peer_downed(peer_id)
	end
end
...
function HuskPlayerMovement:_sync_movement_state_lobby_empty(event_descriptor)
	if self:need_revive() then
...
function HuskPlayerMovement:_sync_movement_state_lobby_weapon(event_descriptor)
	if self:need_revive() then
		self:sync_action_walk_nav_point(nil, "exit_bleedout", sync_action_force_and_execute)
	end
...
function HuskPlayerMovement:_sync_movement_state_lobby(event_descriptor)
	self:_sync_movement_state_standard(event_descriptor)
end
