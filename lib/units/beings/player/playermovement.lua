...
require("lib/units/beings/player/states/PlayerLobby")
...
function PlayerMovement:_setup_states()
	...
	self._states = {
		...
		lobby_empty = PlayerLobbyEmpty:new(unit),
		lobby_weapon = PlayerLobbyWeapon:new(unit),
		lobby = PlayerLobby:new(unit)
	}
end
...
function PlayerMovement:save(data)
	...
	if self._current_state_name ~= "clean" and self._current_state_name ~= "civilian" and self._current_state_name ~= "mask_off" then
		if self._current_state_name == "lobby_empty" then
			-- Nothing
...
function PlayerMovement:update_teleport(t, dt)
	...
	if self._teleport_t and self._teleport_t < t then
		...
		if _G.IS_VR then
			...
			if self._teleport_data.state ~= "lobby_empty" then
				self:current_state():set_base_rotation(self._teleport_data.rotation)
			end
		end
		...
		if managers.player:is_carrying() and not self._teleport_data.skip_drop_carry then
