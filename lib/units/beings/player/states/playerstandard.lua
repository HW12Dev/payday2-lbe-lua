...
function PlayerStandard:_check_action_throw_grenade(t, input)
	...
	local action_forbidden = not PlayerBase.USE_GRENADES or self:chk_action_forbidden("interact") or self._unit:base():stats_screen_visible() or self:_is_throwing_grenade() or self:_interacting() or self:is_deploying() or self:_changing_weapon() or self:_is_meleeing() or self:_is_using_bipod() or tweak_data.vr_arcade.grenades_forbidden
...
function PlayerStandard:_check_action_melee(t, input)
	...
	local action_forbidden = not self:_melee_repeat_allowed() or self._use_item_expire_t or self:_changing_weapon() or self:_interacting() or self:_is_throwing_projectile() or self:_is_using_bipod() or tweak_data.vr_arcade.melee_forbidden
	...
	if col_ray and alive(col_ray.unit) then
		...
		if _G.IS_VR then
			...
			if melee_hand_id then
				custom_data = {
					engine = melee_hand_id == 1 and "right" or "left"
				}
			end
		end
...
function PlayerStandard:_start_action_intimidate(t, secondary)
	if not self._intimidate_t or tweak_data.player.movement_state.interaction_delay < t - self._intimidate_t then
		...
		local voice_type, plural, prime_target = self:_get_unit_intimidation_action(false, not secondary, true, false, true, nil, nil, nil, secondary)
