...
function BaseInteractionExt:only_weapon_hand()
	return self._tweak_data.only_weapon_hand
end
...
HandednessInteractionExt = HandednessInteractionExt or class(UseInteractionExt)

function HandednessInteractionExt:_add_string_macros(macros)
	HandednessInteractionExt.super._add_string_macros(self, macros)

	macros.HAND = managers.localization:text("hud_int_hold_choose_handedness_" .. (self._hand_id == PlayerHand.LEFT and "left" or "right"))
end

function HandednessInteractionExt:interact(player)
	HandednessInteractionExt.super.interact(self, player)

	local hand_trigger_sequence = self._hand_id == PlayerHand.LEFT and "interact_left_hand" or "interact_right_hand"

	self._unit:damage():run_sequence_simple(hand_trigger_sequence)
	managers.blackmarket:apply_loadout_in_game(tweak_data.vr_arcade.loadouts.rifle_american.loadout, nil, function ()
		if _G.IS_VR then
			local hand = self._hand_id == PlayerHand.LEFT and "left" or "right"

			player:hand():set_default_state(hand, "weapon")
		end
	end)
	...
	if _G.IS_VR then
		...
		managers.vr:set_setting("default_weapon_hand", hand)

		local watch_hand = player:hand():hand_unit(PlayerHand.other_hand_id(hand))

		watch_hand:damage():run_sequence_simple("show_watch")
		managers.hud:bind_watch_to_hand(watch_hand)
		managers.hud:setup_floating_hud(self._hand_id)
	end
end

LoadoutSelectionInteractionExt = LoadoutSelectionInteractionExt or class(UseInteractionExt)

function LoadoutSelectionInteractionExt:interact(player)
	LoadoutSelectionInteractionExt.super.interact(self, player)

	local loadout_id = self._tweak_data.loadout

	if not loadout_id then
		debug_pause_unit(self._unit, "[LoadoutSelectionInteractionExt:interact] no loadout id given!", self.tweak_data)

		return
	end

	local loadout = tweak_data.vr_arcade.loadouts[loadout_id] and tweak_data.vr_arcade.loadouts[loadout_id].loadout

	if not loadout then
		debug_pause_unit(self._unit, "[LoadoutSelectionInteractionExt:interact] no loadout called " .. tostring(loadout_id) .. " exists!", self.tweak_data)

		return
	end

	managers.blackmarket:apply_loadout_in_game(loadout, 2)
end
