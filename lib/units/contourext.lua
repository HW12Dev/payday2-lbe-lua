	...
	while self._contour_list and index <= #self._contour_list do
		...
		if data.ray_check and unit:movement() then
			...
			if is_current then
				...
				if cam_pos then
					turn_on = unit:raycast("ray", unit:movement():m_com(), cam_pos, "slot_mask", self._slotmask_world_geometry, "report") and mvector3.distance_sq(cam_pos, unit:movement():m_com()) < 16000000
				end
