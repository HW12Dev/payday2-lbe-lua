		...
		if not Global.exe_arguments_parsed then
			...
			for i = 1, #arg_list do
				...
				elseif arg == "-auto_enter_level" then
					...
					i = i + 1
				elseif arg == "-game_host" or arg == "-host" then
					Global.game_host = true
				elseif arg == "-game_client" or arg == "-client" then
					Global.join_host = arg_list[i + 1]
					i = i + 1
				elseif arg == "-auto_ready" then
					Global.auto_ready = true
				elseif arg == "-restart" then
					Global.was_restarted = true
				elseif arg == "-team_color" then
					local colors = tweak_data.vr_arcade.available_team_colors
					local index = tonumber(arg_list[i + 1])
					Global.team_color = colors[index] or colors[1]
					i = i + 1
				elseif arg == "-preferred_character" then
					local characters = tweak_data.vr_arcade.available_characters
					local index = tonumber(arg_list[i + 1])

					if index ~= 0 then
						Global.preferred_character = characters[index] or characters[1]
					end

					i = i + 1
				elseif arg == "-pod_group" then
					Global.pod_group = tonumber(arg_list[i + 1])
					i = i + 1
				elseif arg == "-pod_nr" then
					Global.pod_nr = tonumber(arg_list[i + 1])
					i = i + 1
				elseif arg == "-autotest_startup" then
					Global.test_startup = {
						restart_time = tonumber(arg_list[i + 1])
					}
					i = i + 1
				end
			end
