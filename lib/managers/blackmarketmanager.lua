...
function BlackMarketManager:init_finalize()
	...
	managers.savefile:add_load_done_callback(callback(self, self, "_savefile_loaded"))
	self:set_arcade_crew()
end

function BlackMarketManager:_savefile_loaded(slot, success, is_setting_slot, cache_only)
	if slot == SavefileManager.PROGRESS_SLOT then
		if Global.game_host then
			self:set_character_configuration(tweak_data.vr_arcade.server_character)
		else
			self:set_character_configuration(Global.preferred_character)
		end
	end
end
...
function BlackMarketManager:preferred_henchmen(index)
	return not index and tweak_data.vr_arcade.available_characters or tweak_data.vr_arcade.available_characters[index]
...
function BlackMarketManager:henchman_loadout(index, should_filter_usage)
	...
	local arcade_loadouts = {
		"rifle_american",
		"rifle_russian",
		"sniper",
		"shotgun"
	}
	local arcade_loadout = tweak_data.vr_arcade.loadouts[arcade_loadouts[index]]
	loadout.primary = arcade_loadout.loadout.primaries.factory_id .. "_npc"
	loadout.blueprint = arcade_loadout.loadout.primaries.blueprint
		...
		for slot, item in pairs(crafted_category) do
			...
			if weapon_invalid then
				...
				for _, part_id in ipairs(duplicate_parts) do
					...
					if table.contains(tweak_data.weapon.factory.allow_multiple_part_types, tweak_data.weapon.factory.parts[part_id].type) then
						remove_part = false
					end
...
function BlackMarketManager:apply_loadout_in_game(loadout, force_selection, done_cb)
	self:clear_in_game_weapons()

	if force_selection then
		managers.player:player_unit():inventory():equip_selection(force_selection)
	end
	...
	for _, category in ipairs({
		...
		local slot = #self._global.crafted_items[category] + 1

		self:craft_in_game(category, loadout[category].weapon_id, slot, loadout[category].blueprint, loadout[category].texture_switches)
		self:equip_weapon_in_game(category, slot, nil, clbk)
	end

	managers.savefile:save_progress()
end

function BlackMarketManager:craft_in_game(category, weapon_id, slot, blueprint, texture_switches)
	if category ~= "primaries" and category ~= "secondaries" then
		return
	end
	...
	self._global.crafted_items[category] = self._global.crafted_items[category] or {}
	local factory_id = managers.weapon_factory:get_factory_id_by_weapon_id(weapon_id)
	local blueprint = clone(blueprint) or deep_clone(managers.weapon_factory:get_default_blueprint_by_factory_id(factory_id))
	self._global.crafted_items[category][slot] = {
		ingame = true,
		weapon_id = weapon_id,
		factory_id = factory_id,
	...
	if texture_switches then
		for part_id, data_string in pairs(texture_switches) do
			self:set_part_texture_switch(category, slot, part_id, data_string)
		end
	end
end

function BlackMarketManager:clear_in_game_weapons()
	local categories = {
		"primaries",
		"secondaries"
	}
...
function BlackMarketManager:set_character_configuration(character, ...)
	if character then
		if character == "hoxton" then
			character = "old_hoxton"
		end

		self:set_preferred_character(CriminalsManager.convert_new_to_old_character_workname(character), 1)
	end

	self:set_arcade_crew()
end

function BlackMarketManager:set_arcade_crew(custom_crew)
	local henchmen = custom_crew or {
		"dallas",
		"chains",
		"wolf",
		"old_hoxton"
	...
	for i, v in ipairs(henchmen) do
		self:set_preferred_henchmen(i, v)
	end
