	...
	local function f(info)
		...
		for i, room in ipairs(room_list) do
			...
			if managers.network.matchmake:is_server_ok(friends_only, room.owner_id, attribute_list[i], nil) then
				...
				if SystemInfo:distribution() == Idstring("STEAM") and Steam:logged_on() and Steam:friends() then
...
function CrimeNetGui:add_special_contract(special_contract, no_casino, no_quickplay)
	...
	local allow = id and not self._jobs[id] and (not special_contract.unlock or special_contract.unlock and tweak_data:get_value(special_contract.id, special_contract.unlock) <= managers.experience:current_level()) and (special_contract.id ~= "casino" or not no_casino) and (special_contract.id ~= "quickplay" or not no_quickplay)
	...
	for id, job in pairs(self._jobs) do
		if job.mouse_over == 1 then
			...
			if not data.dlc or managers.dlc:is_dlc_unlocked(data.dlc) then
				...
				node = node or Global.game_settings.single_player and "crimenet_contract_singleplayer" or job.server and "crimenet_contract_join" or "crimenet_contract_host"
