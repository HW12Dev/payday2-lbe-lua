...
function LootManager._dispose_of_sound(...)
end
...
function LootManager:sync_secure_loot(carry_id, multiplier_level, silent, peer_id)
	...
	if not tweak_data.carry.small_loot[carry_id] then
		...
		if peer_id then
			...
			local position = peer and alive(peer:unit()) and peer:unit():position()

			if position then
				local sound_source = SoundDevice:create_source("LootManager")

				sound_source:set_position(position)
				sound_source:post_event("hlp_poof_small")
				managers.enemy:add_delayed_clbk("LootManager", callback(LootManager, LootManager, "_dispose_of_sound", {
...
function LootManager:loot_by_peer_id(peer_id, small_loot)
	local loot = {}

	for _, data in pairs(self._global.secured) do
		if data.peer_id == peer_id and not not small_loot == not not tweak_data.carry.small_loot[data.carry_id] then
			table.insert(loot, data)
		end
	end
