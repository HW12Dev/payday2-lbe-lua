...
function StatisticsManager:_setup(reset)
	...
	self._defaults.killed_by_peer = {}
	self._defaults.peer_downed = {}
...
function StatisticsManager:killed_by_peer(peer_id)
	self._global.session.killed_by_peer[peer_id] = (self._global.session.killed_by_peer[peer_id] or 0) + 1
end

function StatisticsManager:session_killed_by_peer(peer_id)
	return self._global.session.killed_by_peer[peer_id] or 0
end

function StatisticsManager:killed_by_anyone(data)
	if alive(data.weapon_unit) then
		local user_unit = data.weapon_unit:base()._setup.user_unit

		if alive(user_unit) then
			local peer = managers.network:session():peer_by_unit(user_unit)

			if peer then
				self:killed_by_peer(peer:id())
			end
		end
	end
...
function StatisticsManager:peer_downed(peer_id)
	self._global.session.peer_downed[peer_id] = (self._global.session.peer_downed[peer_id] or 0) + 1
end

function StatisticsManager:session_peer_downed(peer_id)
	return self._global.session.peer_downed[peer_id] or 0
end
