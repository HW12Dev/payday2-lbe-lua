...
function PlayerManager:_internal_load()
	...
	default_weapon_selection = tweak_data.vr_arcade.default_weapon_selection or default_weapon_selection
...
function PlayerManager:drop_carry(zipline_unit)
	...
	if _G.IS_VR then
		...
		if active_hand then
			...
			rotation = active_hand:rotation() * Rotation(0, 0, -90)
			forward = rotation:y()
		end
	end
...
function PlayerManager:sync_carry_data(unit, carry_id, carry_multiplier, dye_initiated, has_dye_pack, dye_value_multiplier, position, dir, throw_distance_multiplier_upgrade_level, zipline_unit, peer_id)
	...
	throw_distance_multiplier = throw_distance_multiplier * 1.5
...
local arcade_to_sync = {
	endscreen = "lobby_empty",
	tutorial = "lobby_empty",
	success = "lobby_empty",
	ingame = "standard",
	lobby = "lobby"
}

function PlayerManager:set_sync_state_from_arcade_state(arcade_state)
	self._current_sync_state = arcade_to_sync[arcade_state] or self._current_sync_state
end
