...
function HUDManager:_update_name_labels(t, dt)
	...
	for _, data in ipairs(self._hud.name_labels) do
		...
		if pos then
			...
			if mvector3.distance_sq(cam_pos, pos) > 16000000 then
				label_panel:set_visible(false)
			end
...
function HUDManager:set_mugshot_weapon(id, hud_icon_id, weapon_index)
	for i, data in ipairs(self._hud.mugshots) do
		if data.id == id then
			...
			if managers.criminals:character_data_by_name(data.character_name_id) then
				local character_data = managers.criminals:character_data_by_name(data.character_name_id)

				if character_data then
					self:_set_teammate_weapon_selected(character_data.panel_id, weapon_index, hud_icon_id)
				end
			end
		...
		elseif data.state == "present" then
			...
			if dot < 0 or panel:outside(mvector3.x(wp_pos), mvector3.y(wp_pos)) then
				...
				if data.distance then
					...
					data.distance:set_y(data.bitmap:center_y() + 16)
				end
			end
		end
