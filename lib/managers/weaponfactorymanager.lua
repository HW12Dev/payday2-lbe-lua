...
function WeaponFactoryManager:_preload_part(factory_id, part_id, forbidden, override, parts, third_person, need_parent, done_cb, async_task_data, only_record)
	...
	if not package then
		...
		if not only_record and not managers.dyn_resource:is_resource_ready(ids_unit, ids_unit_name, managers.dyn_resource.DYN_RESOURCES_PACKAGE) then
			if async_task_data then
...
function WeaponFactoryManager:_add_part(p_unit, factory_id, part_id, forbidden, override, parts, third_person, need_parent, async_task_data)
	...
	if async_task_data then
		...
		if not managers.dyn_resource:is_resource_ready(ids_unit, ids_unit_name, "packages/dyn_resources") then
			...
			self:clbk_part_unit_loaded(async_task_data, true, ids_unit, ids_unit_name)
		end
	else
		if not package and not managers.dyn_resource:is_resource_ready(ids_unit, ids_unit_name, "packages/dyn_resources") then
...
function WeaponFactoryManager:change_part_blueprint_only(factory_id, part_id, blueprint, remove_part)
	...
	if remove_part then
		...
		if table.contains(tweak_data.weapon.factory.allow_multiple_part_types, type) then
			table.insert(blueprint, part_id)
