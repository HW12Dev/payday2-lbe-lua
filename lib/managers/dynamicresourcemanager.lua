...
function DynamicResourceManager:preload_units()
	...
	for _, data in pairs(tweak_data.vr_arcade.loadouts) do
		for _, category in pairs({
			"primaries",
			"secondaries"
		}) do
			local factory_id = data.loadout[category].factory_id
			local blueprint = data.loadout[category].blueprint
			local weapon = tweak_data.weapon.factory[factory_id]
			local ids_unit_name = Idstring(weapon.unit)

			if not self:is_resource_ready(ids_unit, ids_unit_name, self.DYN_RESOURCES_PACKAGE) then
				self:load(ids_unit, ids_unit_name, self.DYN_RESOURCES_PACKAGE, nil)
			end

			managers.weapon_factory:preload_blueprint(factory_id, blueprint, false, true, function ()
			end, false)
		end
	end
end
