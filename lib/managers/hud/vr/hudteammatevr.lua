...
function HUDTeammateVR:init(i, teammates_panel, is_player, width)
	...
	self:override_function_parameters("_create_carry", function (self, carry_panel)
		return self, self._special_equipment_panel
	end)
	...
	self._ammo_panel:set_w(100)
	self._ammo_panel:set_x(200)
...
function HUDTeammateVR:_create_radial_health(radial_health_panel)
	if not self._main_player then
		...
		radial_health_panel:set_size(150, 150)
		radial_health_panel:set_x(30)
...
function HUDTeammateVR:_create_stamina_radial(stamina_panel)
	...
	stamina_panel:set_visible(false)
end
...
function HUDTeammateVR:_create_weapon_panels(weapons_panel)
	if not self._main_player then
		...
		secondary_total:set_visible(false)
		primary_total:set_visible(false)
		secondary_panel:set_h(secondary_panel:h() / 2)
		secondary_panel:set_w(panel:w() - 10)
		secondary_panel:set_x(5)
		primary_panel:set_h(primary_panel:h() / 2)
		primary_panel:set_w(secondary_panel:w())
		primary_panel:set_x(secondary_panel:x())
		primary_panel:set_y(secondary_panel:bottom())
		secondary_clip:set_w(secondary_panel:w())
		primary_clip:set_w(primary_panel:w())
		primary_clip:set_vertical("top")
...
function HUDTeammateVR:_create_reload_panel()
	...
	panel:set_w(self._ammo_panel:w())
...
function HUDTeammateVR:set_name(teammate_name)
	...
	if self._watch_panel:child("character_name") then
		self._watch_panel:remove(self._watch_panel:child("character_name"))
	end

	local character_name = managers.network:session() and managers.network:session():local_peer() and managers.network:session():local_peer():character() or managers.criminals:local_character_name()

	if character_name then
		local char_name = self._watch_panel:text({
			name = "character_name",
			font_size = 18,
			text = managers.localization:to_upper_text("menu_" .. character_name),
			font = tweak_data.hud.medium_font_noshadow,
			color = tweak_data.chat_colors[#tweak_data.chat_colors]
		})

		managers.hud:make_fine_text(char_name)
		char_name:set_center_x(self._watch_panel:w() / 2)
		char_name:set_bottom(self._watch_panel:h() - 15)
	end
end
...
function HUDTeammateVR:set_ability_cooldown(data)
	...
	if self._main_player then
		if data.cooldown and data.cooldown > 0 then
			managers.hud:belt():set_state("throwable", "invalid")
			managers.hud:belt():set_amount("throwable", data.cooldown)
		else
			managers.hud:belt():set_state("throwable", "default")
			managers.hud:belt():set_amount("throwable")
		end
	end
end
...
function HUDTeammateVR:set_hand(hand)
	if PlayerHand.hand_id(hand) == PlayerHand.RIGHT then
		...
		self._ammo_panel:set_x(200)
		self._radial_health_panel:set_x(30)
	else
		self._ammo_panel:set_x(0)
		self._radial_health_panel:set_x(215)
		self._stamina_panel:set_x(215)
		self._radial_health_panel:set_x(115)
		self._stamina_panel:set_x(115)
	end
