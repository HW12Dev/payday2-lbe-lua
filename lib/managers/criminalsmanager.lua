...
function CriminalsManager:get_valid_player_spawn_pos_rot(peer_id)
	if self:has_custom_drop_in_point(peer_id) then
		return self:custom_drop_in_point(peer_id)
	end
...
function CriminalsManager:add_custom_drop_in_point(id, pos, rot)
	self._drop_in_points = self._drop_in_points or {}
	self._drop_in_points[id] = {
		data = {
			pos,
			rot
		}
...
function CriminalsManager:set_custom_drop_in_point_peer_id(id, peer_id)
	if not self._drop_in_points or not self._drop_in_points[id] then
		return
	end

	self:custom_drop_in_point_peer_id_sync(id, peer_id)
	managers.network:session():send_to_peers_synched("sync_drop_in_point_used", id, peer_id)
end

function CriminalsManager:custom_drop_in_point_peer_id_sync(id, peer_id)
	if not self._drop_in_points or not self._drop_in_points[id] then
		return
	end

	self._drop_in_points[id].peer_id = peer_id
end

function CriminalsManager:remove_custom_drop_in_point(id)
	if not self._drop_in_points then
		return
	end

	self._drop_in_points[id] = nil
end

function CriminalsManager:has_custom_drop_in_point(peer_id)
	return not not self:_find_custom_drop_in_point(peer_id)
end

function CriminalsManager:custom_drop_in_point(peer_id)
	local point, id = self:_find_custom_drop_in_point(peer_id)

	if not point then
		Application:error("[CriminalsManager:custom_drop_in_point] failed to find a spawn point for peer:", peer_id)

		return
	end

	self:set_custom_drop_in_point_peer_id(id, peer_id)

	return point.data
end

function CriminalsManager:_find_custom_drop_in_point(peer_id)
	local unused = {}

	if self._drop_in_points then
		for id, point in pairs(self._drop_in_points) do
			if point.peer_id then
				if point.peer_id == peer_id then
					return point, id
				end
			else
		...
		local id = unused[next(unused)]

		return self._drop_in_points[id], id
	end
end

function CriminalsManager:sync_drop_in_points()
	for id, data in pairs(self._drop_in_points or {}) do
		managers.network:session():send_to_peers_synched("sync_drop_in_point", id, data.peer_id, unpack(data.data))
	end
end
