...
function ObjectInteractionManager:interact(player, data, hand_id)
	if alive(self._active_unit) then
		...
		if _G.IS_VR and (interacted or interacted == nil) and player:hand():get_default_hand_id("weapon") == hand_id then
			self:set_hand_temporary_max_distance(hand_id, 0, self._active_unit:position())
		end
...
function ObjectInteractionManager:set_hand_temporary_max_distance(hand_id, distance, pos)
	if not _G.IS_VR then
		return
	end

	self._hand_max_distance = self._hand_max_distance or {}
	self._hand_max_distance[hand_id] = {
		distance,
		pos
	}
end
...
function ObjectInteractionManager:_update_targeted(player_pos, player_unit, hand_unit, hand_id)
	...
	for i = 1, self._close_freq do
		...
		local correct_hand = true

		if _G.IS_VR and (unit:interaction():only_weapon_hand() and true or false) ~= (player_unit:hand():get_default_hand_id("weapon") == hand_id) then
			correct_hand = false
		end

		if alive(unit) and unit:interaction():active() and not self:_in_close_list(unit, hand_id) and correct_hand then
			local distance = mvec3_dis(player_pos, unit:interaction():interact_position())
	...
	for _, unit in ipairs(close_units_list) do
		local distance = mvec3_dis(player_pos, unit:interaction():interact_position())
	...
	if self._hand_max_distance and self._hand_max_distance[hand_id] then
		local dis, pos = unpack(self._hand_max_distance[hand_id])
		local distance = mvec3_dis(player_pos, pos)

		if not closest_distance or distance < closest_distance then
			closest_distance = distance
		end

		self._hand_max_distance[hand_id][1] = math.max(dis, closest_distance - 10)

		if self._hand_max_distance[hand_id][1] < closest_distance then
			close_units_list = {}
		else
			self._hand_max_distance[hand_id] = nil
		end
	end
	...
	if _G.IS_VR then
		if last_active ~= self._active_unit then
			if self._active_unit then
				player_unit:hand():start_show_intrest(not self._active_unit:interaction():can_interact(), hand_id)
