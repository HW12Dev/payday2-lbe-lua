...
function HUDManager:loot_value_updated()
	...
	if self:ingame_workspace("van_screen") then
		self:_update_van_screen()
	end
end
...
function HUDManager:on_workspace_created_van_screen(workspace)
	local panel = workspace:panel()
	local bag_text = panel:text({
		name = "bag_text",
		font = "fonts/big_letters",
		text = "THROW BAG IN VAN",
		font_size = tweak_data.menu.pd2_massive_font_size * 2
	})

	self:make_fine_text(bag_text)
	bag_text:set_center(panel:w() / 2, 550)
	...
	arrow:set_center_x(bag_text:center_x())
	arrow:set_y(bag_text:bottom() + 10)
	panel:set_visible(false)
end

function HUDManager:set_van_screen_visible(visible)
	if not self:ingame_workspace("van_screen") then
		return
	end

	self:ingame_workspace("van_screen"):panel():set_visible(visible)
end

function HUDManager:_update_van_screen()
	local my_loot = managers.loot:loot_by_peer_id(managers.network:session():local_peer():id())

	if #my_loot == 0 then
		return
	end

	local panel = self:ingame_workspace("van_screen"):panel()
	local loot_amount = managers.loot:get_secured_bonus_bags_amount()

	if loot_amount > 0 then
		local text = string.format("%02d BAGS IN VAN", loot_amount)

		if loot_amount == 1 then
			text = string.gsub(text, "BAGS", "BAG ")
		end

		local bag_text = panel:child("bag_text")

		bag_text:set_text(text)
		self:make_fine_text(bag_text)
		bag_text:set_center_x(panel:w() / 2)
	end
end

function HUDManager:on_workspace_created_lbe_main_screen(workspace)
	local panel = workspace:panel()
	local bg = panel:bitmap({
		texture = "units/pd2_dlc_vr/menu/lbe_screen_preplanning_df",
		name = "preplanning"
	})
	local size = workspace:panel():w() + 128 * workspace:panel():w() / 1920

	bg:set_size(size, size)
end

function HUDManager:on_workspace_created_lbe_left_screen(workspace)
	local panel = workspace:panel()
	local bg = panel:bitmap({
		texture = "units/pd2_dlc_vr/menu/lbe_screen_assets_df",
		name = "assets"
	})
	local size = workspace:panel():w() + 128 * workspace:panel():w() / 1920
	...
	self._peer_score_needs_clearing = true
end
...
function HUDManager:set_arcade_score(score, cash, stats)
	local score_ws = self:ingame_workspace("lbe_main_screen")

	if not score_ws then
		return
	end

	local score_panel = score_ws:panel()

	score_panel:clear()

	local bg = score_panel:bitmap({
		texture = "units/pd2_dlc_vr/menu/lbe_screen_logo_df",
		name = "bg",
		layer = BG_LAYER1
	})
	local size = score_panel:w() + 128 * score_panel:w() / 1920

	bg:set_size(size, size)

	local total_score = score_panel:text({
		font = "fonts/big_numbers",
		font_size = tweak_data.menu.pd2_massive_font_size * 2,
		text = managers.experience:cash_string(score),
		layer = TEXT_LAYER
	})

	self:make_fine_text(total_score)
	total_score:set_center(score_panel:w() / 2, score_panel:h() * 0.4)

	local detail_panel = score_panel:panel({
		name = "total",
		w = score_panel:w() * 0.8,
		h = score_panel:h() * 0.4,
		y = score_panel:h() * 0.56,
		layer = OBJECT_LAYER
	})

	detail_panel:set_center_x(score_panel:w() / 2)
	make_bg_panel(detail_panel)
	...
	local detail_y = detail_panel:h() / 20

	for _, stat_name in ipairs(stat_order) do
		local stat_value = stats[stat_name]

		if stat_value and (stat_name ~= "hostages_remaining" or stat_value > 0) then
			local score_value = managers.job:score_from_stat(stat_name, stat_value)
			local color = score_value < 0 and Color.red or Color.white
			local skip_amount = false
			local stat_name_text_id = "hud_score_" .. stat_name

			if stat_name == "downed" and stat_value == 0 then
				stat_name_text_id = "hud_score_no_down_bonus"
				skip_amount = true
			end

			local stat_name_text = detail_panel:text({
				name = stat_name .. "_title",
				font = tweak_data.menu.pd2_medium_font,
				font_size = tweak_data.menu.pd2_medium_font_size * 1.5,
				text = managers.localization:to_upper_text(stat_name_text_id),
				x = detail_panel:w() / 20,
				y = detail_y,
				color = color,
				layer = TEXT_LAYER
			...
			self:make_fine_text(stat_name_text)

			if not skip_amount then
				local stat_amount_text = detail_panel:text({
					name = stat_name .. "_amount",
					font = tweak_data.menu.pd2_medium_font,
					font_size = tweak_data.menu.pd2_medium_font_size * 1.5,
					text = tostring(stat_value) .. "x",
					y = detail_y,
					color = color,
					layer = TEXT_LAYER
				})

				self:make_fine_text(stat_amount_text)
				stat_amount_text:set_right(detail_panel:w() * 0.49)

				local stat_value_text = detail_panel:text({
					name = stat_name .. "_value",
					font = tweak_data.menu.pd2_medium_font,
					font_size = tweak_data.menu.pd2_medium_font_size * 1.5,
					text = managers.experience:cash_string(tweak_data.vr_arcade.score_multipliers[stat_name], ""),
					x = detail_panel:w() / 2,
					y = detail_y,
					color = color,
					layer = TEXT_LAYER
				...
				self:make_fine_text(stat_value_text)
			end

			local stat_score_text = detail_panel:text({
				name = stat_name .. "_score",
				font = tweak_data.menu.pd2_medium_font,
				font_size = tweak_data.menu.pd2_medium_font_size * 1.5,
				text = managers.experience:cash_string(score_value),
				y = detail_y,
				color = color,
				layer = TEXT_LAYER
			})

			self:make_fine_text(stat_score_text)
			stat_score_text:set_right(detail_panel:w() - detail_panel:w() / 20)

			detail_y = detail_y + stat_score_text:h() * 1.2
		end
	end
end

local function format_stat(name, stat)
	if name == "accuracy" then
		return tostring(stat) .. "%"
	elseif name == "cash" then
		return managers.experience:cash_string(stat)
	end

	return tostring(stat)
end

function HUDManager:set_arcade_peer_stats(peer_id, data)
	local is_dedicated_host = Network:is_server() and Global.game_host or Network:is_client() and managers.network:session():server_peer():server_only()
	local id = is_dedicated_host and peer_id - 2 or peer_id - 1
	local is_local = managers.network:session():local_peer():id() == peer_id
	local peer_score_ws = self:ingame_workspace("lbe_left_screen")

	if not peer_score_ws then
		return
	end

	local peer_score_panel = peer_score_ws:panel()

	if self._peer_score_needs_clearing then
		peer_score_panel:clear()

		self._peer_score_needs_clearing = nil
	end

	local name_id = "peer_" .. tostring(peer_id)

	if peer_score_panel:child(name_id) then
		peer_score_panel:remove(peer_score_panel:child(name_id))
	end

	if not peer_score_panel:child("bg") then
		local bg = peer_score_panel:bitmap({
			texture = "units/pd2_dlc_vr/menu/lbe_screen_logo_df",
			name = "bg",
			layer = BG_LAYER1
		})
		local size = peer_score_panel:w() + 128 * peer_score_panel:w() / 1920

		bg:set_size(size, size)
	end

	local padding = 5
	local w_padding = peer_score_panel:w() * 0.01
	local panel_w = peer_score_panel:w() * 0.18
	local panel = peer_score_panel:panel({
		name = name_id,
		x = (panel_w + w_padding) * id + w_padding + panel_w,
		y = peer_score_panel:h() * 0.3,
		w = panel_w - w_padding,
		h = peer_score_panel:h() * 0.65,
		layer = OBJECT_LAYER
	})
	panel_w = panel:w()

	if is_local then
		local border = panel:bitmap({
			texture = "guis/dlcs/vr/textures/pd2/border",
			name = "border",
			layer = BG_LAYER1
		})
		local offset_ratio = 20 / border:texture_width()

		border:set_w(panel:w() + panel:w() * offset_ratio * 2)
		border:set_h(panel:h() + panel:h() * offset_ratio * 2)
		border:set_x(-panel:w() * offset_ratio)
		border:set_y(-panel:h() * offset_ratio)
		border:set_rotation(360)
	end

	local character_name = managers.network:session():peer(peer_id) and managers.network:session():peer(peer_id):character() or "russian"
	local character_text = panel:text({
		name = "character",
		align = "center",
		font = tweak_data.menu.pd2_large_font,
		font_size = tweak_data.menu.pd2_large_font_size,
		text = managers.localization:to_upper_text("menu_" .. character_name),
		y = padding,
		layer = TEXT_LAYER
	})

	self:make_fine_text(character_text)
	character_text:set_w(panel_w)

	local peer_loadout = "rifle_american"
	local primary_factory_id = managers.network:session():peer(peer_id) and managers.network:session():peer(peer_id):blackmarket_outfit().primary.factory_id

	for loadout_name, loadout in pairs(tweak_data.vr_arcade.loadouts) do
		if loadout.loadout.primaries.factory_id == primary_factory_id then
			peer_loadout = loadout_name

			break
		end
	end

	local loadout_text = panel:text({
		name = "loadout",
		font_size = 32,
		align = "center",
		font = tweak_data.menu.pd2_large_font,
		text = managers.localization:to_upper_text(tweak_data.vr_arcade.loadouts[peer_loadout].name_id),
		y = character_text:bottom() + padding,
		layer = TEXT_LAYER
	})

	self:make_fine_text(loadout_text)
	loadout_text:set_w(panel_w)
	...
	local stats_panel = panel:panel({
		name = "stats",
		y = loadout_text:bottom() + padding
	})

	stats_panel:set_h(panel:h() - stats_panel:y())
	make_bg_panel(stats_panel)

	local stat_font_size = tweak_data.menu.pd2_large_font_size * 0.9

	local function calc_y(i, y, enlarge)
		local font_size = stat_font_size

		if enlarge then
			font_size = font_size * 1.8
		end

		local h = stats_panel:h() + padding - (y or 0)
		local count = #stat_order - i + 1
		local stat_padding = h / count - stat_font_size

		return y or stat_padding / 2, stat_padding, font_size
	end

	if not peer_score_panel:child("stat_names") then
		local stat_names_panel = peer_score_panel:panel({
			name = "stat_names",
			y = panel:y(),
			x = w_padding,
			w = panel_w - w_padding,
		...
		local loadout_title = stat_names_panel:text({
			text = "LOADOUT",
			name = "loadout_title",
			font_size = 32,
			align = "center",
		...
		self:make_fine_text(loadout_title)
		loadout_title:set_w(panel_w)

		local offset = loadout_title:bottom() + padding
		local y = nil

		for i, name in ipairs(stat_order) do
			local stat_padding, font_size = nil
			y, stat_padding, font_size = calc_y(i, y, name == "score")
			local stat = data[name]
			local stat_name_text = stat_names_panel:text({
				name = name,
				font = tweak_data.menu.pd2_large_font,
				font_size = stat_font_size,
				text = managers.localization:to_upper_text("hud_score_" .. name),
				y = y + (font_size - stat_font_size) / 2 + offset,
				layer = TEXT_LAYER
			})

			self:make_fine_text(stat_name_text)
			stat_name_text:set_center_x(panel_w / 2)

			y = y + font_size + stat_padding
		end
	end
	...
	for i, name in ipairs(stat_order) do
		...
		y, stat_padding, font_size = calc_y(i, y, name == "score")
		local stat = data[name]
		local stat_text = stats_panel:text({
			name = name .. "_value",
			font = tweak_data.menu.pd2_large_font,
			font_size = font_size,
			text = format_stat(name, stat),
			y = y,
			layer = TEXT_LAYER
		})

		self:make_fine_text(stat_text)
		stat_text:set_center_x(panel_w / 2)

		y = y + stat_text:h() + stat_padding
	end
end

function HUDManager:highlight_peer_stats(highlights)
	local peer_score_ws = self:ingame_workspace("lbe_left_screen")
	...
	for peer_id, stats in pairs(highlights) do
		local name_id = "peer_" .. tostring(peer_id)
		local stats_panel = peer_score_panel:child(name_id) and peer_score_panel:child(name_id):child("stats")
		local stat_order = {
			"score",
			"accuracy",
			"kills",
		...
		for _, stat in ipairs(stat_order) do
			...
			local stat_obj = alive(stats_panel) and stats_panel:child(stat .. "_value")

			if alive(stat_obj) then
				stat_obj:set_color(color)
			end
		end
	end
end
