...
function HUDManagerVR:on_flick(dir, time)
	...
	for _, panel in ipairs(self._tablet_ws:panel():children()) do
...
function HUDManagerVR:link_ammo_hud(hand_unit, side)
	...
	if side == 1 or side == "right" then
		self._ammo_ws:set_linked(300, 200, hand_obj, hand_obj:position() + Vector3(-20, 5, 5):rotate_with(hand_rot), Vector3(15, 0, 0):rotate_with(hand_rot), Vector3(0, -2.6, -9.65):rotate_with(hand_rot))
		self._controller_assist_panel:set_x(100)
	elseif side == 2 or side == "left" then
		self._ammo_ws:set_linked(300, 200, hand_obj, hand_obj:position() + Vector3(5, 5, 5):rotate_with(hand_rot), Vector3(15, 0, 0):rotate_with(hand_rot), Vector3(0, -2.6, -9.65):rotate_with(hand_rot))
...
function HUDManagerVR:link_watch_prompt(hand_unit, side)
	...
	if side == 1 or side == "right" then
		self._watch_prompt_ws:set_linked(800, 80, hand_obj, hand_obj:position() + Vector3(5.4, 0, 25.8):rotate_with(hand_rot), Vector3(24, 0, -41.6):rotate_with(hand_rot), Vector3(-3.84, -1.6, -2.24):rotate_with(hand_rot))
	elseif side == 2 or side == "left" then
		self._watch_prompt_ws:set_linked(800, 80, hand_obj, hand_obj:position() + Vector3(-27, 0, -10):rotate_with(hand_rot), Vector3(24, 0, 41.6):rotate_with(hand_rot), Vector3(3.84, -1.6, -2.24):rotate_with(hand_rot))
	end
end
...
function HUDManagerVR:bind_hud_to_vr_hand(weapon_hand_unit, tablet_hand_unit, belt_unit, weapon_side, tablet_side)
	...
	self._prompt_ws:set_linked(600, 150, tablet_object, tablet_object:position() + Vector3(-7.5, -15, -3.5), Vector3(0, 30, 0), Vector3(7.5, 0, 0))
...
function HUDManagerVR:link_floating_hud(float_unit)
	...
	local y_vec = Vector3(0, -0.7, -0.7):rotate_with(rot) * size
...
function HUDManagerVR:setup_floating_hud(weapon_hand)
	local panel = self._floating_ws:panel()

	panel:clear()
	panel:bitmap({
		depth_mode = "disabled",
		texture = "guis/dlcs/vr/textures/pd2/move_shoot_" .. (weapon_hand == PlayerHand.LEFT and "l" or "r")
	})
end

function HUDManagerVR:set_floating_hud_visible(visible)
	if self._floating_ws:panel():visible() == visible then
		return
	end
	...
	self._floating_ws:panel():stop()
	self._floating_ws:panel():animate(visible and show or hide)
end
...
function HUDManagerVR:create_vehicle_interaction_ws(id, vehicle_unit, position, direction, up)
	...
	local size = Vector3(10, 0, -10)
	self._vehicle_interactions[id] = self._gui:create_linked_workspace(100, 100, vehicle_unit:orientation_object(), vehicle_unit:orientation_object():position() + position:rotate_with(vehicle_rot) - size:rotate_with(ws_rot) / 2 - direction * 5, ws_rot:x() * size.x, ws_rot:z() * size.z)
...
function HUDManager:add_waypoint(id, data)
	...
	local ws = self._gui:create_world_workspace(128, 96, (data.position or data.unit:position()) + Vector3(40, 0, 20), Vector3(-80, 0, 0), Vector3(0, 0, -60))
	...
	bitmap:set_size(bitmap:w() * 2, bitmap:h() * 2)
	arrow:set_size(arrow:w() * 2, arrow:h() * 2)
	...
	local function wp_anim(o)
		...
		while alive(o) do
			...
			if progress < 2 then
				s = math.sin(progress * 540) + 1
			else
				s = math.sin(progress * 180) / 8 + 1
			end
	...
	local function wp_anim_world(o)
		if not alive(o) then
			return
		end
		...
		while alive(o) do
			...
			o:set_center(parent:w() / 2, parent:h() / 2)
		end
	end

	bitmap:animate(wp_anim)
	bitmap_world:animate(wp_anim_world)
end
		...
		elseif data.state == "present" then
			...
			if dot < 0 or panel:outside(mvector3.x(wp_pos), mvector3.y(wp_pos)) then
				...
				if data.in_timer ~= 1 then
					...
					data.bitmap_world:set_center(data.ws:panel():w() / 2, data.ws:panel():h() / 2)
				else
