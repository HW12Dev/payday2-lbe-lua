...
function VRLoadingEnvironment:_start_enter(start_type)
	...
	VRManager:fade_to_color(0, Color(1, 0, 0, 0), true)
	VRManager:suspend_rendering(true)

	local loading_screen = nil

	if start_type == "end" then
		loading_screen = "loading_arcade_end"
	else
		loading_screen = "loading_arcade"
	end

	self:_show_loading_screen(loading_screen)
	self:_fade_overlays(0)
end

function VRLoadingEnvironment:_start_exit()
	VRManager:set_skybox_override({
		Idstring("guis/dlcs/vr/textures/loading/loading_bg_blurred_df")
	})
	VRManager:suspend_rendering(true)
end
